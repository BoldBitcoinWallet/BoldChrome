 ...
 * Generates first 10 addresses of each type (legacy, segwit-nested, segwit-native)
 */
export async function deriveInitialAddresses() {
  ...
  try {
    console.log('[Wallet] Deriving addresses from public key...');
    
    // Derive addresses for all types
    const derived = hdWallet.deriveAllTypes(
      { publicKey, chainCode, network },
      10 // Derive 10 addresses per type
    );

    // Convert to DerivedAddress format with type information
    ...
    // Automatically select first native segwit address (most modern)
...
export async function refreshWalletData() {
  ...
  try {
    // Fetch balance and transactions from mempool.space
    ...
    const transactions: Transaction[] = txHistory.map((tx: any) => {
      ...
      const amount = voutSum - vinSum;
      const type: Transaction['type'] = amount > 0 ? 'receive' : 'send';
      ...
      return {
        ...
        from: type === 'receive' ? tx.vin[0]?.prevout?.scriptpubkey_address : currentAddress,
        to: type === 'send' ? tx.vout[0]?.scriptpubkey_address : currentAddress,
      };
    });
