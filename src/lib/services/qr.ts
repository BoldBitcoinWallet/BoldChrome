...
class QRService {
   ...
   */
  async generatePairingQR(): Promise<{ dataUrl: string; payload: string }> {
    ...
    return { dataUrl: qrCodeDataUrl, payload };
  }
   ...
   * send flow. Format: <to_address>|<amount_satoshi>|<fee_satoshi>|<spendingHash>
   */
  async generateSendQR(toAddress: string, amountSats: number | string, feeSats: number | string, spendingHash: string = ''): Promise<{ dataUrl: string; payload: string }> {
    const id = `send-${Date.now()}-${Math.random().toString(36).substring(7)}`;

    const payload = `${toAddress}|${amountSats}|${feeSats}|${spendingHash || ''}`;
  ...
  async processScanedQR(qrText: string): Promise<{ type: QRDataType; data: any }> {
    try {
      ...
      // Heuristic: compact base64 pairing payload (mobile QR encoded compactly)
      // Example mobile payload (base64): Au8Hbv0koeOpNgjfn5uf6TUqfLvG2lCF16G1pjsJmCwAL27VFypJq+xDrM0nXvQduUh94bIHJ0eNZPm0HTxEWlI43g==
      // Decoded hex layout: <pubkey(66 hex chars)><chaincode(64 hex chars)><checksum(4 hex chars)>
      const base64Candidate = qrText.trim();
      if (/^[A-Za-z0-9+/]+=*$/.test(base64Candidate) && base64Candidate.length >= 80 && base64Candidate.length <= 140 && !base64Candidate.startsWith('cHNidP8')) {
        try {
          const hex = this.base64ToHex(base64Candidate);
          if (hex && hex.length >= (66 + 64 + 4)) {
            const pubKeyHex = hex.substring(0, 66);
            const chainCodeHex = hex.substring(66, 66 + 64);
            const checksum = hex.substring(66 + 64, 66 + 64 + 4);

            // pairing_code can be configured via chrome.storage.local.pairingCode, otherwise fallback to extension id
            const stored = await new Promise<Record<string, any>>(resolve => chrome.storage.local.get(['pairingCode'], resolve));
            const pairingCode: string = (stored && stored.pairingCode) || chrome.runtime?.id || 'extension-uuid-code';

            const hash = CryptoJS.SHA256(pubKeyHex + chainCodeHex + pairingCode).toString(CryptoJS.enc.Hex);
            const expected = hash.substring(0, 4);

            if (checksum.toLowerCase() === expected.toLowerCase()) {
              console.log('[QR] Detected compact base64 pairing payload and checksum validated');
              const pairingData = { publicKey: pubKeyHex, chainCode: chainCodeHex, deviceId: 'mobile-qr', network: 'mainnet' };
              const processed = await this.processPairingData(pairingData);
              return { type: 'pairing_response', data: processed };
            } else {
              console.warn('[QR] Compact pairing payload checksum mismatch', { checksum, expected });
            }
          }
        } catch (err) {
          console.warn('[QR] Failed to decode compact base64 pairing payload', err);
        }
      }
  ...
  // Helper to convert base64 string to hex string (browser-safe)
