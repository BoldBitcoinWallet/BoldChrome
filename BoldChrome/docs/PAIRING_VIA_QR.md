...
## Bold bind flow (swimlanes.io, recommended)

1. **Extension** creates a pairing code and shows a QR with `pairing_code=XXXX` (e.g. 6 digits).
2. **Mobile** (Bold Wallet app) scans that QR, user confirms, mobile builds response (cipher + checksum), shows response QR.
3. **Extension** scans the mobile response QR (base64, 67 bytes). It deciphers with the stored pairing code, extracts `pub_key` and `chain_code`, validates the checksum, and stores the wallet.

The response format is binary (not JSON): 65-byte cipher (payload XOR sha256(pairing_code)) + 2-byte checksum, base64-encoded. The extension parses this in `extensionBind.ts`; the mobile generates it in `utils/extensionBind.ts` (BoldWallet repo).
